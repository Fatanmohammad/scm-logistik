<x-app-layout>
    <div class="py-10" x-data="{ activeTab: '{{ $errors->updatePassword->isNotEmpty() ? 'password' : ($errors->userDeletion->isNotEmpty() ? 'delete' : ($errors->isNotEmpty() ? 'profile' : 'none')) }}' }">
        
        @if($errors->any())
        <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-4" x-init="setTimeout(() => show = false, 6000)" class="fixed bottom-6 right-6 z-50 bg-red-600 text-white px-6 py-4 rounded-2xl shadow-2xl shadow-red-600/30 flex items-start gap-3 max-w-sm">
            <svg class="w-6 h-6 shrink-0 text-red-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            <div>
                <h4 class="font-bold text-sm tracking-wide">Terjadi Kesalahan!</h4>
                <ul class="text-xs opacity-90 mt-1.5 space-y-1">
