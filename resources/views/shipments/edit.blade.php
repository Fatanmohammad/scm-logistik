<x-app-layout>
    <div class="min-h-screen bg-slate-50/50 py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-black text-slate-800 tracking-tight">Edit Pengiriman</h2>
                    <p class="text-sm text-slate-500">Perbarui data pengiriman di sistem SCM.</p>
                </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/60 border border-white overflow-hidden">
                <form action="{{ route('shipments.update', $shipment) }}" method="POST" class="p-8 sm:p-12">
                    @csrf
                    @method('PUT')
                        ...
                        <section>
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="tracking_code" value="{{ old('tracking_code', $shipment->tracking_code) }}"
                                    ...
                                    @error('tracking_code')<p class="text-red-500 text-xs mt-2 ml-1 font-semibold">{{ $message }}</p>@enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="destination" rows="3"
                                        class="w-full px-5 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:border-slate-900 focus:ring-0 transition-all shadow-sm resize-none" required>{{ old('destination', $shipment->destination) }}</textarea>
                                    @error('destination')<p class="text-red-500 text-xs mt-2 ml-1 font-semibold">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </section>
                                ...
                                <div>
                                    ...
                                    <select name="product_id" id="product_select" class="w-full px-5 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:border-slate-900 focus:ring-0 transition-all shadow-sm" required>
                                        ...
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}" data-stock="{{ $product->stock + ($shipment->product_id == $product->id && $shipment->status != 'delivered' ? $shipment->quantity : 0) }}" {{ old('product_id', $shipment->product_id) == $product->id ? 'selected' : '' }}>
                                                {{ $product->name }} (SKU: {{ $product->sku }}) — Stok: {{ $product->stock }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('product_id')<p class="text-red-500 text-xs mt-2 ml-1 font-semibold">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-slate-400 uppercase tracking-wider mb-2 ml-1">
                                        Jumlah Kirim
                                    </label>
                                    <input type="number" name="quantity" id="quantity_input" value="{{ old('quantity', $shipment->quantity) }}" min="1" placeholder="0"
                                        class="w-full px-5 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:border-slate-900 focus:ring-0 transition-all shadow-sm" required>
                                    <p id="stock_info" class="text-[10px] font-bold text-slate-400 mt-2 ml-1">Pilih produk untuk melihat stok tersedia.</p>
                                    @error('quantity')<p class="text-red-500 text-xs mt-2 ml-1 font-semibold">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-slate-400 uppercase tracking-wider mb-2 ml-1">
                                    ...
                                    <select name="user_id" class="w-full px-5 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:border-slate-900 focus:ring-0 transition-all shadow-sm" required>
                                        ...
                                        @foreach($staffUsers as $staff)
                                            <option value="{{ $staff->id }}" {{ old('user_id', $shipment->user_id) == $staff->id ? 'selected' : '' }}>
                                                {{ $staff->name }} (Staf)
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('user_id')<p class="text-red-500 text-xs mt-2 ml-1 font-semibold">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    ...
                                    <select name="status" class="w-full px-5 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:border-slate-900 focus:ring-0 transition-all shadow-sm" required>
                                        ...
                                        <option value="on_delivery" {{ old('status', $shipment->status) == 'on_delivery' ? 'selected' : '' }}>On Delivery</option>
                                        <option value="delivered"   {{ old('status', $shipment->status) == 'delivered'   ? 'selected' : '' }}>Delivered</option>
                                    </select>
                                    @error('status')<p class="text-red-500 text-xs mt-2 ml-1 font-semibold">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </section>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            updateStockInfo(); // Call once on load to set initial state
        });
    </script>
</x-app-layout>
